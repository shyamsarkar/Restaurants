...
class PDF_MC_Table extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(30, 5, 'Bill Number', 1, 0, 'L', 0);
		$this->Cell(30, 5, 'Bill Date', 1, 0, 'C', 0);
		$this->Cell(30, 5, 'Time', 1, 0, 'C', 0);
		$this->Cell(35, 5, 'Table', 1, 0, 'C', 0);
		$this->Cell(65, 5, 'Total Amount', 1, 1, 'R', 0);

		$this->SetX(5);
		$this->SetAligns(array('L', 'C', 'C', 'C', 'C', 'R'));
		$this->SetWidths(array(10, 30, 30, 30, 35, 65));
...
$slno = 1;
$grand_total = 0;
$res = $obj->executequery("Select * from bills $crit and checked_nc=1 order by billid desc");
foreach ($res as $row_get) {
	...
	$pdf->Row(array($slno++, $row_get['billnumber'], $billdate, $billtime, strtoupper($table_no), number_format(round($net_bill_amt), 2)));
...
$pdf->Cell(170, 5, 'NC Total (Rs.): ', 1, 0, 'R', 0);
$pdf->Cell(30, 5, number_format(round($grand_total), 2), 1, 1, 'R', 0);
$pdf->Ln(2);
$pdf->SetX(5);
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(15, 5, 'Remark: ', 0, 0, 'L', 0);
$pdf->MultiCell(180, 5, $remarks, 0, 'L');
$pdf->Output();
?> 
