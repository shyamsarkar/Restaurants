...
foreach ($res as $rowget) {
    ...
    $saleno = $rowget['saleno'];
    $sale_type = $rowget['sale_type'];
    ...
    $mobile = $obj->getvalfield("master_customer", "mobile", "customer_id='$customer_id'");
    ...
    $term_cond = $obj->getvalfield("master_customer", "term_cond", "customer_id='$customer_id'");
    $gsttinNno = $obj->getvalfield("company_setting", "gsttinno", "company_id='$company_id'");
    $caddress = $obj->getvalfield("company_setting", "address", "company_id='$company_id'");
...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
        global $title1, $title2, $company_name, $customer_name, $billno, $sale_date, $address, $gsttinno, $gsttinNno, $gsttinno, $company_address, $comp_mobile, $address, $company_code, $title3, $title4, $tinno, $address2, $comp_name, $mobile, $tinno, $openingbal, $saleno, $sale_type, $term_cond, $gsttinNno, $caddress, $remark;
        //courier 25
        ...
        $this->Cell(90);
        $this->Ln(5);
        $this->Line(5, 28, 145, 28); //1st
        $this->Line(5, 33, 145, 33); //2nd

        $this->Line(75, 28, 75, 100); //3rd
        ...
        $this->SetX(5);
        $this->SetFont('courier', 'b', 9);
        $this->Cell(10, 5, "Challan NO", 0, 0, 'L');
        $this->SetY(53);
        $this->SetX(25);
        $this->SetFont('courier', 'b', 9);
        $this->Cell(3, 5, ":", 0, 0, 'L');
        ...
        $this->SetY(48);
        $this->SetX(105);
        $this->MultiCell(40, 5, $caddress, 0, 'L');
        ...
        $this->Cell(100, 5, 'Description', 1, 0, 'L', 1);
        $this->Cell(33, 5, 'Qnty', 1, 1, 'R', 1);
        //$this->Cell(15,5,'Rate',1,0,'C',1);
        //$this->Cell(26,5,'Amount',1,1,'C',1);
        $this->SetWidths(array(7, 100, 33));
...
foreach ($res as $row_get) {
    ...
    //$totalamt +=$amount;
    //$grandtotal = $totalamt + $openingbal;
    $totalqty += $qty;
    $slno++;
}
...
$pdf->SetTextColor(0, 0, 0);
$pdf->SetX(90);
$pdf->Cell(10, 5, "Total Quantity :", 0, 0, 'L');
$pdf->Cell(45, 5, $totalqty, 0, 0, 'R');
$pdf->Ln(5);
$pdf->Output();
?> 
