...
$res = $obj->executequery("select * from demand_entry where demand_id='$_GET[demand_id]'");
foreach ($res as $rowget) {
    //$company_id = $_SESSION['company_id'];
    //$customer_id= $_SESSION['customer_id'];
    //$purchasedate= $obj->dateformatindia($rowget['purchasedate']);
    $demand_order_no = $rowget['demand_order_no'];
    ...
    $demand_date = $obj->dateformatindia($rowget['demand_date']);
...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
        global $title1, $title2, $company_name, $customer_name, $demand_order_no, $purchasedate, $address, $gsttinno, $gsttinNno, $company_address, $mobile, $company_code, $company_state, $demand_date, $gsttinno, $challan_no, $title3, $title4, $tinno, $address1, $comp_name, $mobile, $tinno, $date, $date1, $supplier_name;
        //courier 25
        ...
        $this->SetY(5);
        $this->SetFont('courier', 'b', 9);
        $this->Cell(28, 51, "Demand To -", 0, 0, 'L');
        $this->Cell(68, 51, '', 0, 0, 'L');
        //for Supplier Name
        $this->SetFont('courier', 'b', 9);
        $this->SetX(110);
        $this->Cell(33, 51, "Demand From -", 0, 0, 'L');
        ...
        $this->Cell(71, 10, ":" . $supplier_name, 0, 0, 'L');
        //for Supplier Name
        $this->SetFont('courier', 'b', 9);
        ...
        $this->SetY(8);
        $this->SetFont('courier', 'b', 9);
        $this->Cell(30, 70, "Demand No", 0, 0, 'L');
        $this->Cell(71, 70, ":" . $demand_order_no, 0, 0, 'L');
        //for Supplier GST
        $this->SetFont('courier', 'b', 9);
        ...
        $this->SetY(8);
        $this->SetFont('courier', 'b', 9);
        $this->Cell(30, 80, "Demand Date", 0, 0, 'L');
        $this->Cell(71, 80, ":" . $demand_date, 0, 0, 'L');
        ...
        $this->Cell(86, 5, 'Product Name', 1, 0, 'L', 1);
        $this->Cell(60, 5, 'UNIT', 1, 0, 'L', 1);
        $this->Cell(40, 5, 'QTY', 1, 1, 'C', 1);

        $this->SetX(5);

        $this->SetWidths(array(14, 86, 60, 40));
...
$slno = 1;
$total_qty = 0;
$res = $obj->executequery("Select * from demand_detail where demand_id='$demand_id'");
foreach ($res as $row_get) {
    ...
    $pdf->Row(array($slno, strtoupper($product_name), strtoupper($unit_name), $qty));
...
// $pdf->Cell(13,7,'',1,0,'R',1);
// $pdf->Cell(13,7,'',1,0,'R',1);
$pdf->Cell(40, 7, number_format($total_qty, 2), 1, 0, 'R', 1);
...
// $pdf->Cell(200,5,'Total Amount in Words : '.ucfirst(convert_number_to_words(round($net_total))). " Rupees ONLY",0,1,'L',0);
$pdf->Ln(5);
$pdf->Output();
?> 
