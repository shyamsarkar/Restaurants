...
$raw_id= addslashes($_REQUEST['raw_id']);
...
if($raw_id !='' && $qty !='')
{
	...
	// else
	// {
	// $form_data = array('inc_or_exc'=>$inc_or_exc,'product_id'=>$product_id,'unit_name'=>$unit_name,'qty'=>$qty,'rate_amt'=>$rate_amt,'cgst'=>$cgst,'sgst'=>$sgst,'igst'=>$igst,'purchaseid'=>$purchaseid,'ipaddress'=>$ipaddress,'createdate'=>$createdate,'createdby'=>$loginid,'disc'=>$disc);
