...
foreach ($res as $rowget) {
    ...
    //    $openingbal = $obj->getvalfield("master_customer","openingbal","customer_id='$customer_id'");
...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
        global $title1, $title2, $company_name, $customer_name, $billno, $sale_date, $address, $gsttinno, $gsttinNno, $company_address, $comp_mobile, $address, $company_code, $company_state, $cus_code, $cus_state, $challan_no, $title3, $title4, $tinno, $address2, $comp_name, $mobile, $tinno, $openingbal, $saleno;
        //courier 25
        ...
        $this->SetX(5);
        $this->SetFont('courier', 'b', 9);
        $this->Cell(20, 5, "Bill To", 0, 0, 'L');
        $this->Cell(71, 5, "", 0, 0, 'L');
        //for Supplier Name
        $this->SetX(75);
        $this->SetFont('courier', 'b', 9);
        $this->Cell(20, 5, "Bill From", 0, 0, 'L');
        ...
        $this->SetY(33);
        $this->SetX(30);
        $this->Cell(45, 5, $customer_name, 0, 0, 'L');
        ...
        $this->SetY(58);
        $this->SetX(29);
        // //$this->Cell(45,5,$sale_date,0,0,'L');
        ...
        $this->SetY(63);
        $this->SetX(29);
        // //$this->MultiCell(45,5,$address,0,'L');
        ...
        $this->Cell(70, 5, 'Description', 1, 0, 'L', 1);
        $this->Cell(15, 5, 'Qnty', 1, 0, 'R', 1);
        $this->Cell(15, 5, 'Rate', 1, 0, 'R', 1);
        $this->Cell(26, 5, 'Amount', 1, 1, 'R', 1);
        $this->SetWidths(array(14, 70, 15, 15, 26));
...
foreach ($res as $row_get) {
    ...
    $pdf->Row(array($slno, $prodname, $qty, number_format($rate, 2), number_format($amount, 2)));
    $totalamt += $amount;
    $totgst += $totalc;
    $totigst += $totali;
    $totsgst += $totals;
...
$pdf->SetTextColor(0, 0, 0);
$pdf->Cell(109, 8, 'Total', 0, 0, 'R', 0);
$pdf->SetX(116);
$pdf->Cell(3, 8, ':', 0, 0, 'L', 0);
$pdf->SetX(135);
$pdf->Cell(10, 8, number_format($totalamt, 2), '0', 1, 'R', 0);
if ($totgst != '') {
    $pdf->Ln(0);
    $pdf->SetX(5);
    $pdf->SetFont('arial', 'b', 9);
    $pdf->SetTextColor(0, 0, 0);
    $pdf->Cell(109, 8, 'CGST Amt', 0, 0, 'R', 0);
    $pdf->SetX(116);
    $pdf->Cell(3, 8, ':', 0, 0, 'L', 0);
    $pdf->SetX(135);
    $pdf->Cell(10, 8, number_format($totgst, 2), '0', 1, 'R', 0);
}
if ($totsgst != '') {
    $pdf->Ln(0);
    $pdf->SetX(5);
    $pdf->SetFont('arial', 'b', 9);
    $pdf->SetTextColor(0, 0, 0);
    $pdf->Cell(109, 8, 'SGST Amt', 0, 0, 'R', 0);
    $pdf->SetX(116);
    $pdf->Cell(3, 8, ':', 0, 0, 'L', 0);
    $pdf->SetX(135);
    $pdf->Cell(10, 8, number_format($totsgst, 2), '0', 1, 'R', 0);
}
if ($totigst != '') {
    $pdf->Ln(0);
    $pdf->SetX(5);
    $pdf->SetFont('arial', 'b', 9);
    $pdf->SetTextColor(0, 0, 0);
    $pdf->Cell(109, 8, 'IGST Amt', 0, 0, 'R', 0);
    $pdf->SetX(116);
    $pdf->Cell(3, 8, ':', 0, 0, 'L', 0);
    $pdf->SetX(135);
    $pdf->Cell(10, 8, number_format($totigst, 2), '0', 1, 'R', 0);
}
$pdf->Ln(0);
$pdf->SetX(5);
$pdf->SetFont('arial', 'b', 9);
$pdf->SetTextColor(0, 0, 0);
$pdf->Cell(109, 8, 'Grand Total', 0, 0, 'R', 0);
$pdf->SetX(116);
$pdf->Cell(3, 8, ':', 0, 0, 'L', 0);
$pdf->SetX(135);
$pdf->Cell(10, 8, number_format(round($totalamount), 2), '0', 1, 'R', 0);
...
$pdf->Cell(200, 5, 'Total Amount in Words  :' . ucfirst(convert_number_to_words(round($totalamount))) . " ONLY", 0, 1, 'L', 0);
$pdf->Ln(5);
