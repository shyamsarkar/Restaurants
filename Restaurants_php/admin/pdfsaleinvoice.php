...
foreach ($res as $row_get) {
    ...
    $hsnno = $obj->getvalfield("m_product", "hsnno", "product_id='$product_id'");
    ...
    $pdf->Row(array($slno, $prodname, $hsnno, $qty, $unit_name, number_format($rate, 2), number_format($amount, 2), $disc . '%', strtoupper(substr($inc_or_exc, 0, 2)) . '-' . $taxable_value, $cgst . '%', $cgst_amt, $sgst . '%', $sgst_amt, $igst . '%', $igst_amt, number_format($final_price, 2)));
    $tot_amt += $amount;
    $totcgst += $totalc;
    $totsgst += $totals;
    $totigst += $totali;
