...
$pdf->Cell(95,6,'Floor Name',1,0,'L',0);
$pdf->Cell(95,6,'Table No.',1,1,'L',0);
...
$floor_name = "";
        ...
        $sql1 = $obj->executequery("select * from m_table order by table_id desc");
       foreach ($sql1 as $sql) {
        //print_r($sql);
        $floor_id = $sql['floor_id'];
        $floor_name = $obj->getvalfield("m_floor","floor_name","floor_id='$floor_id'");
    $pdf->SetX(5);  
    $pdf->SetFont('Arial','',8);
    $pdf->SetTextColor(0,0,0);
    $pdf->Row(array($slno++,$floor_name,$sql['table_no']));
}
