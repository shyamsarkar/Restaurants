...
$pagename = "nc_product_report.php";
$module = "NC Product Report";
$submodule = "NC Product Report";
$btn_name = "Save";
$keyvalue =0 ;
$tblname = "";
$tblpkey = "";
...
if(isset($_GET['table_id']))
{
  ...
  if($table_id!='')
  {
     $crit .= "and bills.table_id='$table_id'";
  }
}
       ...
       <div class="contentinner content-dashboard">
        ...
        <form method="get" action="">
          <table class="table table-bordered table-condensed">
            ...
            <tr>
              ...
              <td><input type="text" name="from_date" id="from_date" class="input-medium"  placeholder='dd-mm-yyyy'
               ...
               <td><input type="text" name="to_date" id="to_date" class="input-medium"  placeholder='dd-mm-yyyy'
                 ...
                 <td>&nbsp; <a href="nc_product_report.php"  name="reset" id="reset" class="btn btn-success">Reset</a></td>
               </tr>
             </table>
           ...
           <div>
      <!-- for print pdf -->
       <?php $chkview = $obj->check_pageview("nc_product_report.php",$loginid);             
                    if($chkview == 1 || $_SESSION['usertype']=='admin'){  ?>
         <p align="right" style="margin-top:7px; margin-right:10px;"> <a href="pdf_nc_product.php?from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&table_id=<?php echo $table_id; ?>" class="btn btn-info" target="_blank">
          ...
          <table class="table table-bordered" id="dyntable">
            <thead>
              <tr>
                ...
                <th style="text-align: right;" class="head0">NC_Amount</th>
              </tr>
            </thead>
            <tbody id="record">
            ...
            $res = $obj->executequery("select bills.billdate,bills.nc_amount, bill_details.* from bill_details left join bills on bills.billid = bill_details.billid $crit and bills.checked_nc='1'");
            ...
            foreach($res as $row_get)
            {
              ...
              $nc_amount =$row_get['nc_amount'];
              $billnumber = $obj->getvalfield("bills","billnumber","billid='$billid'");
              
              ?> <tr>
                <td><?php echo $slno++; ?></td>
                <td> <a href="pdf_restaurant_recipt.php?billid=<?php echo $row_get['billid'] ?>" target="_blank"><?php echo $billnumber; ?></a></td>
                <td><?php echo $obj->dateformatindia($billdate); ?></td>
                <td><?php echo strtoupper($prodname); ?></td>
                <td><?php echo strtoupper($unit_name); ?></td>
                <td><?php echo $qty; ?></td>
                <td><?php echo $table_no; ?></td>
                <td style="text-align: right;"><?php echo number_format($nc_amount,2); ?></td>
                
              </tr>
              <?php
            $totnc_amount += $nc_amount;
              }//looop close

              ?>
            </tbody>
          </table>
          <div class="well well-sm text"><h3 class="text-info text-right">Total NC Amount: <?php echo number_format($totnc_amount,2); ?></h3></div> 
         <?php } ?>
        </div><!--contentinner-->
      </div><!--maincontent-->
    </div>
