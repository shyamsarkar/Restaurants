...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
        global $title1, $title2, $waiter_name, $year, $month_name;
        // courier 25
        ...
        $this->Cell(88, 8, 'Date', 1, 0, 'L', 0);
        $this->Cell(88, 8, 'Status', 1, 0, 'L', 0);
        $this->Cell(88, 8, 'Time', 1, 1, 'L', 0);



        $this->SetWidths(array(23, 88, 88, 88));
...
$tot = 0;
$tot_present = 0;
//echo "select * from emp_attendance_entry $crit2";die;
...
for ($d = 1; $d <= $month_days; $d++) {
    ...
    $aday = date($month);
    $attendance_time = $obj->getvalfield("emp_attendance_entry", "attendance_time", "waiter_id='$waiter_id' and emp_attendance_date='$mydate'");
    $attendanceday = $obj->getvalfield("emp_attendance_entry", "count(emp_attendance_date)", "waiter_id='$waiter_id' and DATE_FORMAT(emp_attendance_date,'%m')='$aday'");
    $searh_year_month = $year . '-' . $month;

    //$presentdays = $attendanceday + $count_sunday;
    ...
    $pdf->Row(array($slno++, $obj->dateformatindia($mydate), $atype, $attendance_time));
}
